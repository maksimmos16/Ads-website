...
$output .='<div class="dhe-form-checkbox'.(!empty($inline_css) ? ' dhe-form-checkbox__custom_w':'').(!empty($conditional) ? ' dhe-form-conditional':'').'">'."\n";
if(!empty($options_list)){
	$options_arr = json_decode(base64_decode($options_list));
	global $dhe_form;
	$options_arr = apply_filters('dhe_form_checkbox_options', $options_arr,$dhe_form,$name);
	...
	if(!empty($options_arr)){
		...
		foreach ((array)$options_arr as $option){
			$id = uniqid('_');
			$label_image = '';
			$label_class = 'dhe-form-checkbox__option-label';
			...
			$output .= '<input data-field-name="'.$name.'" data-name="'.$name.'" '.(!empty($conditional) ? 'data-conditional-name="'.$name.'" data-conditional="'.esc_attr(base64_decode($conditional)).'"': '' ).' type="checkbox" '.(!empty($disabled) ? ' disabled':'').' class="dhe-form-value dhe-form-control-'.$name.' '.(!empty($required) && $i ==0 ? 'dhe-form-required-entry':'').'"  id="dhe_form_control_'.sanitize_title($option->option_value).$id.'" '.($option->option_default === 'yes' ? 'checked="checked"' :'').' name="'.$checkbox_name.'" value="'.esc_attr($option->option_value).'">';
			$output .= $label_image;
			$output .= '<i class="dhe-icon-checkbox"></i>';
