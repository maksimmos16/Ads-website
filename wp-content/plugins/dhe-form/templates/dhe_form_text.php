...
$default_value = apply_filters('dhe_form_text_default_value', $default_value,$dhe_form,$name);
$data_math_field = '';
...
$label = $control_label;
$field_type = apply_filters('dhe_form_text_field_type', $field_type,$dhe_form,$name);
...
$output .= '<input '.$data_math_field.' data-field-name="'.$name.'" autocomplete="off" type="'.$field_type.'" id="dhe_form_control_'.$name.'" name="'.$name.'" value="'.$default_value.'" '.(!empty($minlength) ? ' minlength="'.$minlength.'"' : '').' '.(!empty($maxlength) ? ' maxlength="'.$maxlength.'"' : '')
		.' class="dhe-form-control dhe-form-control-'.$name.' dhe-form-value'.(!empty($required) ? ' dhe-form-required-entry':'').' '.(!empty($validator) ? str_replace(',', ' ', $validator):'').'" '.(!empty($required) ? ' required aria-required="true"':'').' '.($readonly =='yes' ? ' readonly':'').' placeholder="'.$placeholder.'" '.$attributes.'>' . "\n";
$output .=$icon_html;
$output .='</div>';
if(!empty($help_text)){
