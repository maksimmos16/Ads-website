 ...
 * @param jQuery
 */
window.dhe_form_recatptcha_widgets = [];
!function( $ ) {
	"use strict"; // jshint ;_;
	
	$(document).ready(function () {
		
		$('[data-auto-open].dhe-form-popup').each(function(){
			var $this = $(this),
				id = $this.attr('id'),
			...
			open_timeout = setTimeout(function(){
				...
				if(one_time){
					if(!$.cookie(id)){
						$('.dhe-form-pop-overlay').show();
						$('body').addClass('dhe-form-opening');
						$this.show();
		...
		$(document).on('click','[data-toggle="dheformpopup"],[rel="dheformpopup"],[href^="#dheformpopup-"]',function(e){
			e.stopPropagation();
			e.preventDefault();
			var href;
			var $this = $(this);
			...
			$target.off('click').on('click',function(e){
				...
				$('.dhe-form-pop-overlay').hide();
				$('body').removeClass('dhe-form-opening');
				$target.hide();
			});
		});
		
		$(document).on('click','.dhe-form-popup-close',function(e){
			$('.dhe-form-pop-overlay').hide();
			$('body').removeClass('dhe-form-opening');
			$(this).closest('.dhe-form-popup').hide();
		});
		
		
		$('.dhe-form-slider-control').each(function(){
			var $this = $(this);
			$this.slider({
				 min: $this.data('min'),
			     max: $this.data('max'),
		...
		var get_field_val = function(field, form){
			var $form = $(form),
				$this=$(field);
			return $this.is(':checkbox') ? $.map($form.find('[data-conditional-name=' + $this.data('conditional-name') + '].dhe-form-value:checked'),
	                function (element) {
						return $(element).val();
	            	})
	            : ($this.is(':radio') ? $form.find('[data-conditional-name=' + $this.data('conditional-name') + '].dhe-form-value:checked').val() : $this.val() );
		...
		var conditional_hook = function(e){
			var $this = $(e.currentTarget),
				...
				container_class = dhe_form_params.container_class,
	       ...
	       is_empty = $this.is(':checkbox') ? !form.find('[data-conditional-name=' + $this.data('conditional-name') + '].dhe-form-value:checked').length
                 : ( $this.is(':radio') ? !form.find('[data-conditional-name=' + $this.data('conditional-name') + '].dhe-form-value:checked').val() : !master_value.length )  ;
	        ...
	        if(is_empty){
	        	$.each(conditional_data,function(i,conditional){
	        		...
	        		$.each(elements,function(index,element){
						var $this = form.find('.dhe-form-control-'+element);
						$this.closest(container_class).addClass('dhe-form-hidden');
						$this.trigger('change');
					});
	        	});
	        	$.each(conditional_data,function(i,conditional){
		        	...
		        	if(conditional.type == 'is_empty'){
		        		if(conditional.action == 'hide'){
							...
							$.each(elements,function(index,element){
								var $this = form.find('.dhe-form-control-'+element);
								$this.closest(container_class).removeClass('dhe-form-hidden');
								$this.trigger('change');
							});
						}
		        	}
				...
				$.each(conditional_data,function(i,conditional){
					...
					if(master_container.hasClass('dhe-form-hidden')) {
						$.each(elements,function(index,element){
							var $this = form.find('.dhe-form-control-'+element);
							$this.closest(container_class).addClass('dhe-form-hidden');
							$this.trigger('change');
		...
		var update_hidden_fields = function(form){
			var $form = $(form);
			var fields = [];
			$form.find('.dhe-form-value').filter(function(){
			...
			$form.find('#_dhe_form_hidden_fields').val(JSON.stringify(fields))
		}
		
		var conditional_form = function(form,ignore_bind){
			var $form = $(form),
				master_box = $form.find('.dhe-form-conditional'),
				ignore_bind = ignore_bind || false;
			
			if('yes'===dhe_form_params.is_preview_mode)
				return;
			
			$.each(master_box,function(){
				var masters = $(this).find('[data-conditional].dhe-form-value');
		...
		var field_calculation = function(form){
			...
			$('[data-field_calculation]',$form).each(function(){
				...
				if(!$.isNumeric(operators)){
					...
					if($this.hasClass('dhe-form-control-label')){
						$this.html(match_value);
					}else{
						$this.val(match_value).prop('value',match_value);
					}
					$( document.body ).trigger( 'dhe_field_calculation_change', [$form] );
				}
			});
		}
		...
		var paypal_calculation = function(form){
			...
			$('.dhe-paypal-calculation',$form).each(function(){
				var match,
					match_value=0,
					$this = $(this),
					pattern = /\[(.*?)\]/g,
				...
				if(!$.isNumeric(operators)){
					...
					$( document.body ).trigger( 'dhe_paypal_calculation_change', [$form] );
				}
			});
			
		}
		
		$(document.body).on('dhe_paypal_calculation_change', function(event, form) {
			var pp_total = 0,
				$form = $(form);
		...
		var initRecaptcha = function(){
			var $elements = $('[data-dheform-recaptcha="recaptcha"]');
			...
			var addRecaptcha = function($elements){
				$elements.each(function(){
					...
					if($this.hasClass('dhe-form-recaptcha2')){
						...
						$(document.body).on('dhe_form_submit', function(event, form, data) {
							switch ( data.status ) {
		...
		if($.fn.xdsoftDatetimepicker && 'function' === typeof $.xdsoftDatetimepicker.setLocale){
			$.xdsoftDatetimepicker.setLocale(dhe_form_params.datetimepicker_lang);
		}
		
		var form_submit_loading = function(form,loaded){
			loaded = loaded || false;
			var $form = $(form);
			var submit = $form.find('.dhe-form-submit');
			var button_label = $form.find('.dhe-form-submit-label');
			var ajax_spinner = $form.find('.dhe-form-submit-spinner');
			if(loaded){
				submit.removeAttr('disabled');
				button_label.removeClass('dhe-form-submit-label-hidden');
	        	ajax_spinner.hide();
			}else{
				submit.attr('disabled','disabled');
	        	button_label.addClass('dhe-form-submit-label-hidden');
	        	ajax_spinner.show();
			}
		}
		
		if($('.dhe-form-datepicker').length){
			$('.dhe-form-datepicker').each(function(){
				var _this = $(this);
		...
		if($('.dhe-form-timepicker').length){
			$('.dhe-form-timepicker').each(function(){
				var _this = $(this);
				_this.xdsoftDatetimepicker({
		...
		$('.dhe-form-control[data-range_field]').each(function(){
			var $this = $(this),
		...
		if($('.dhe-form-datetimepicker').length){
			$('.dhe-form-datetimepicker').each(function(){
				var _this = $(this);
				_this.xdsoftDatetimepicker({
		...
		$('.dhe-form-minicolors').each(function(){
			$(".dhe-form-control",$(this)).minicolors({
				theme: 'bootstrap'
			});
		});
		...
		var initForm = function(form){
			...
			var submitBtn = $form.find('.dhe-form-submit');
			
			$('.dhe-form-file',$form).find('input[type=file]').on('change',function(){
				var _val = $(this).val();
				$(this).closest('label').find('.dhe-form-control').prop('value',basename(_val));
			});
			$('.dhe-form-file',$form).each(function(){
				$(this).find('input[type="text"]').css({'padding-right':$(this).find('.dhe-form-file-button').outerWidth(true) + 'px'});
			...
			if($.fn.tooltip)
				$('.dhe-form-rate .dhe-form-rate-star',$form).tooltip({ html: true,container:$('body')});
			...
			var clearResponse = function(form){
				...
				$( '.dhe-form-error', $form ).remove();
				$( '.dhe-form-control', $form ).removeClass( 'dhe-form-not-valid' );
				$( '.dhe-form-message', $form.parent() )
				.hide().empty().removeAttr( 'role' )
				.removeClass('dhe-form-validation-errors dhe-form-spam dhe-form-errors dhe-form-success');
			}
			...
			var refill = function(form, data){
				...
				var refillCaptcha = function( $form, items ) {
					$.each( items, function( i, n ) {
						...
						$form.find( 'img.dhe-form-captcha-img-' + i ).attr( 'src', n );
						var match = /([0-9]+)\.(png|gif|jpeg)$/.exec( n );
						$form.find( 'input:hidden[name="_dhe_form_captcha_challenge_' + i + '"]' ).attr( 'value', match[ 1 ] );
					} );
				};
				if ( data.captcha ) {
			...
			var notValidTip = function(target, message){
				if(message=='')
					return;
				var $target = $( target );
				$( '.dhe-form-error', $target ).remove();
				var error = $( '<span role="alert" class="dhe-form-error"></span>' );
				
				error.text(message);
				
				if ( $target.is( ':radio' ) || $target.is( ':checkbox' ) )
					error.appendTo( $target.parent().parent() );
				else if($target.attr('data-dheform-recaptcha')=='recaptcha')
					error.appendTo($target.closest('.dhe-form-group') );
				else
					error.appendTo( $target.parent().parent());
			}
			...
			var form_step_click_init = function(form){
				var $form = $(form);
				$('.dhe-form-step',$form).on('click',function(e){
					var $this = $(this);
					e.stopPropagation();
					e.preventDefault();
					...
					$( '.dhe-form-message.dhe-form-success', $form.parent() )
					.hide().empty().removeAttr( 'role' )
					.removeClass('dhe-form-validation-errors dhe-form-spam dhe-form-errors dhe-form-success');
					
					var click_step = parseInt($(this).data('step-index')),
						current_step = parseInt($('#_dhe_form_current_step',$form).val());
					//step control
					$this.siblings('.dhe-form-step.active').removeClass('active').addClass('actived');
					//$('.dhe-form-steps',$form).find('.active').removeClass('active');
					$this.removeClass('actived').addClass('active');
					
					//step content
					$('.dhe-form-step-content',$form).removeClass('active');
					$('.dhe-form-step-content-'+click_step,$form).addClass('active');
					
					$('#_dhe_form_current_step',$form).val(click_step);
					
					$('.dhe-form-value',$form).on('input.dheform change.dheform',function(){
						var current_content_step = $(this).closest('.dhe-form-step-content').data('content_step'),
			...
			var ajaxSuccess = function(data, status, xhr, $form ){
				var $message = $( '.dhe-form-message', $form.parent() );
				...
				switch ( data.status ) {
					case 'validation_failed':
						...
						$.each( data.invalid_fields, function( i, n ) {
							...
							$( '.dhe-form-control', $(n.into).closest('.dhe-form-group') ).addClass( 'dhe-form-not-valid' );
						...
						$message.addClass( 'dhe-form-validation-errors' );
						
						$form.addClass( 'invalid' );
						
						$( document.body ).trigger( 'dhe_form_invalid', [$form, data] );
					break;
					case 'success':
						if($form.find('#_dhe_form_steps').length){
							var step_final = '<div class="dhe-form-steps-final"></div>';
							$form.find('.dhe-form-step-contents').append($(step_final));
						}
						if ( data.onOk ) {
							$.each( data.onOk, function( i, n ) { eval( n ) } );
						}
						$message.addClass( 'dhe-form-success' );
						$( document.body ).trigger( 'dhe_form_success', [$form, data] );
					break;
					case 'spam':
						$message.addClass( 'dhe-form-spam' );
						$( document.body ).trigger( 'dhe_form_spam', [$form, data] );
					break;
					case 'upload_failed':
						$message.addClass( 'dhe-form-errors' );
						$( document.body ).trigger( 'dhe_form_upload_failed', [$form, data] );
					break;
					case 'form_not_exist':
						$message.addClass( 'dhe-form-errors' );
						$( document.body ).trigger( 'dhe_form_not_exist', [$form, data] );
					break;
					case 'action_failed':
						$message.addClass( 'dhe-form-errors' );
						$( document.body ).trigger( 'dhe_form_action_failed', [$form, data] );
					break;
					case 'call_action_failed':
						$message.addClass( 'dhe-form-errors' );
						$( document.body ).trigger( 'dhe_form_call_action_failed', [$form, data] );
					break;
					case 'next_step':
						var $current_step_input = $('#_dhe_form_current_step',$form);
						var $current_step = parseInt($current_step_input.val());
						var $all_steps = parseInt($('#_dhe_form_steps',$form).val());
						var $next_step = $current_step + 1;
						
						if($next_step<=$all_steps ){
							$('.dhe-form-steps',$form).find('.active').removeClass('active');
							$('.dhe-form-step-'+$current_step,$form).addClass('actived');
							$('.dhe-form-step-'+$next_step,$form).removeClass('actived').addClass('active');
							
							$('.dhe-form-step-content',$form).removeClass('active');
							$('.dhe-form-step-content-'+$next_step,$form).addClass('active');
							$('#_dhe_form_current_step',$form).val($next_step);
						}
					break;
				}
				...
				$( document.body ).trigger( 'dhe_form_submit', [$form, data] );
				...
				if(!data.redirect && data.message!=''){
					...
					if($form.find('.dhe-form-steps-final').length){
						$('.dhe-form-step-content').each(function(){
							$(this).remove();
						})
						$form.find('.dhe-form-steps-final').html($message.clone());
						$message.remove();
					}else{
			...
			$form.submit( function( event ) {
				if ('yes'===dhe_form_params.is_preview_mode ||  submiting || typeof window.FormData !== 'function' ) {
					return;
				}
				...
				$.ajax( {
					type: 'POST',
					url:dhe_form_params.ajax_submit_url,
					data: formData,
					dataType: 'json',
					processData: false,
					contentType: false,
					beforeSend: function(){
						$( document.body ).trigger( 'dhe_form_before_submit', [$form, submitBtn] );
				    	form_submit_loading($form,false);
			        }
				} ).done( function( data, status, xhr ) {
					$( document.body ).trigger( 'dhe_form_after_submit', [ $(form), submitBtn , data] );
		...
		$('form.dheform').each(function(){
			var $form = $(this);
			paypal_calculation($form);
			field_calculation($form);
		    $('.dhe-form-value',$form).on('keyup change',function(e){
		    	paypal_calculation($(this).closest('form'));
		    	field_calculation($(this).closest('form'));
			});
		    conditional_form($form);
		    if($form.hasClass('dheform-action-default'))
		    	initForm($form);
		});

		$( document ).on( 'elementor/popup/show',function(event, id){
			$('#elementor-popup-modal-'+id).find('form.dheform').each(function(){
				var $form = $(this);
				paypal_calculation($form);
				field_calculation($form);
			    $('.dhe-form-value',$form).on('keyup change',function(e){
