...
if(!class_exists('DHEFormQuery')){
	require_once (DHE_FORM_DIR.'/includes/db.php');
}
if(!function_exists('dhe_form_is_xhr')){
	require_once (DHE_FORM_DIR.'/includes/functions.php') ;
}

function dhe_form_output_csv($data) {
...
if ( isset($_GET['form_id']) && $_GET['form_id'] != '0' )
{
	...
	foreach ($forms as $form){
		...
		if($form_control){
			...
			$entries = $dheform_db->get_entries($form->ID,'submitted','desc',0);
			...
			foreach ($entries as $i=>$entry){
				...
				foreach ($form_control_arr as $control){
					if ($control['control_name'] !='' && $control['tag'] !='dhe_form_recaptcha' && $control['tag'] != 'dhe_form_captcha' && $control['tag'] != 'dhe_form_submit_button'){
