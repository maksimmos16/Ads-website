	...
	function process_admin_options() {
		...
		if($this->is_user_registered){
			...
			if(strlen($api_sub_key) !== 32){
				$data['api_user'] = ''; $key_not_valid = true;
			}
			if(strlen($api_key) !== 32 && $mode == 2){
				$data['api_key'] = ''; $key_not_valid = true;
			}
			...
			$registrationResult = $this->registration($post_data[$this->wc_field_pref . 'user_name'], 
	...
	*/
	public function admin_options() {
		...
		echo '<table class="form-table" style="'.$display_class.'" id="mtn_partner_portal_data"><tbody>
			...
			<tr valign="top">
				...
				<td class="forminp">
					<p class="description"><i>Use these URLs on MTN Partner Portal -> API access</i></p>
				</td>
			</tr>
		</tbody></table>';
	}
	...
	protected function validation($api_sub_key, $api_user, $api_key, $mode) {
		...
		if( $this->currency['code'] != 'EUR' && $mode == 1) {
			$currencyError = sprintf(__("eShop currency %s is not supported! Sandbox only supports EUR, set under WooCoomerce->Currency options", $this->domain), $this->currency['code']);
			$settings->add_error($currencyError);
			return false;
		} else if(!$this->isLiveCurrency($this->currency['code'])
			&& $mode == 2) {
			//do not allow live mode if not live currency!
			$currencyError = sprintf(__("eShop currency %s is not supported for the Live usage!", $this->domain), 
				$this->currency['code']);
			$settings->add_error($currencyError);
			return false;
		} 
	...
	public function setUserData($data) {	
		...
		if(@$data[$this->wc_field_pref . 'api_user']) $this->api_user = $data[$this->wc_field_pref . 'api_user'];
	}
	...
	public function updateUserData() {
		if( ($res = $this->getUserData()) === false )  return;
	...
	public function receipt_page($order_id) {
		...
		if($this->isActiveGateway() && 
			...
			if ($order->get_payment_method() == 'momopay') {
				echo WC_MomoPay_View::createReceiptBox($order, $payment_id, $site_URL);
			}
		}
	}
