...
class WC_MomoPay_View {
		...
		return '<table class="form-table" data-momopay-step="1">
				...
				<tr valign="top">
					<th scope="row" class="titledesc">
						<label for="woocommerce_momopay_user_email">Merchant Email address</label>
					</th>
					<td class="forminp">
						<fieldset>
	...
	public static function createFinishBox($cfg) {
		$currency = self::getCurrencyInfo($cfg);
		$remaining_requests = $cfg->max_request_limit - $cfg->user_request_counter;
		$promo_box_class = ($remaining_requests) ? 'momopay_limits-box' : 'momopay_limits-box_error';
		$promo_option = '';

		if($cfg->max_request_limit == 999){
			$promo_box_class = 'momopay_limits-box';
			$promo_text = "MOMO FREE GATEWAY";
		} else {
			$promo_text = $cfg->max_request_limit .' requests limit ('. $remaining_requests .' requests left)';
			$promo_option = '<p><a class="button-primary" target="_blank" href="'. self::$pro_plugin_link .'">Upgrade</a></p>';
		}
		...
		if($currency['code'] == 'EUR') $html .= '<p class="mtn-momo-pay_error-box">Sandbox mode! No real payments can be done, no USSD push to mobile phone.</p>';
		else $html .= '<p class="mtn-momo-pay_error-box">You can only request money in this currency!</p>';
	...
	public static function createReceiptBox($order, $payment_id, $urls) {
		...
		$html = '<div class="momopay-checkout-box">
