...
function archive_tags_page_function($public_handler, $search_args, $shortcode_atts, $map_args){
	...
	if ($tag_object = get_term_by('slug', get_query_var('tag-directorypress'), DIRECTORYPRESS_TAGS_TAX)) {
				...
				if(isset($DIRECTORYPRESS_ADIMN_SETTINGS['archive_page_style'])){
					$public_handler->template = 'partials/archive/tags-archive-'. $DIRECTORYPRESS_ADIMN_SETTINGS['archive_page_style'] .'.php';
				}else{
					$public_handler->template = 'partials/archive/tags-archive-1.php';
				}
