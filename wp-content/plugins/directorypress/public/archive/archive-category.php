...
function archive_category_page_function($public_handler, $search_args, $shortcode_atts, $map_args){
	...
	if ($category_object = directorypress_get_term_by_path(get_query_var('category-directorypress'))) {
				...
				if(isset($DIRECTORYPRESS_ADIMN_SETTINGS['archive_page_style'])){
					$public_handler->template = 'partials/archive/category-archive-'. $DIRECTORYPRESS_ADIMN_SETTINGS['archive_page_style'] .'.php';
				}else{
					$public_handler->template = 'partials/archive/category-archive-1.php';
				}
