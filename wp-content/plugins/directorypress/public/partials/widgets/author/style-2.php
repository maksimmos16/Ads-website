	...
	*/
	...
	echo '<div class="directorypress-author style2 clearfix">';
		...
		echo '<div class="author-content">';
			echo '<p class="author-name">'.$author_name.'</p>';
			...
			if($DIRECTORYPRESS_ADIMN_SETTINGS['author_profile_view']){
				echo '<div class="author-link"><a href="'. home_url('/profile/') . $author_nicename .'" class="">'. esc_html__('view all ads', 'DIRECTORYPRESS').'</a></div>';
			}
	...
	echo '</div>';
	if($hide_from_anonymous && !is_user_logged_in()){
		echo '<p class="alert alert-info">'. esc_html__('Ploease login to access contact information', 'DIRECTORYPRESS') .'</p>';		
	}else{
		if(!empty($email_id) && $show_email){
			echo '<div class="author-email-id style2">';
				echo '<i class="fas fa-envelope-open-text"></i>';
				echo '<p class="email-id">'.$email_id.'</p>';
			echo '</div>';
		}
		if(!empty($phone_number) && $show_phone_number){
			echo '<div class="author-phone style2">';
				...
				echo '<a id="number" data-pre="'.$numer_pre.'" data-last="'.$phone_number.'" class="" href="tel:'.$phone_number.'"><i class="fas fa-phone"></i>';
					echo '<span class="number-main pre">'.$numer_pre.'</span>';
					echo '<span class="number-main post" style="display:none;">'.$phone_number.'</span>';
				echo '</a>';
			echo '</div>';
		}
		if(!empty($whatsapp_number) && $show_whatsapp_number){			
			echo '<div class="author-phone whatsapp style2">';
				...
				echo '<a id="whatsapp-number" data-pre="'.$whatsapp_numer_pre.'" data-last="'.$whatsapp_number.'" class="" href="https://api.whatsapp.com/send?phone='.$whatsapp_number.'"><i class="fab fa-whatsapp"></i>';
					echo '<span class="number-main pre">'.$whatsapp_numer_pre.'</span>';
					echo '<span class="number-main post" style="display:none;">'.$whatsapp_number.'</span>';
				echo '</a>';
			echo '</div>';			
		}
		if($DIRECTORYPRESS_ADIMN_SETTINGS['directorypress_listing_contact'] && ($show_contact || $show_offer_button)){
			echo '<div class="author-btns style2">';
				if($show_contact){
					echo '<div class="author-btn-holder '.$contact_button_class.'"><a class="" data-popup-open="single_contact_form" href="#">'.esc_html__('Send message', 'DIRECTORYPRESS').'</a></div>'; 
				}
				if($show_offer_button){
					echo '<div class="author-btn-holder '.$contact_button_class.'"><a data-popup-open="single_contact_form_bid" href="#" class="">'. esc_html__('Send Offer', 'DIRECTORYPRESS').'</a></div>'; 
				}
			echo '</div>';
		}
	}