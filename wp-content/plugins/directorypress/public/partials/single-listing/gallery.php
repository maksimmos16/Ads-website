...
<div class="directorypress-listing-gallery-wrapper">
	<div class="directorypress-gallery-field-name"><?php echo esc_html__('Gallery', 'DIRETORYPRESS'); ?></div>
	<div class="directorypress-listing-gallery">
			<?php
				foreach ($listing->images AS $attachment_id=>$image):
					...
					$images = '<div><a class="slide-link" href="' . $image_src_array[0] . '" data-lightbox="listing_images" title="images"><i class="directorypress-fic4-zoom-out"></i><img class="" data-lazy="' . bfi_thumb($image_src, $param) . '" width="'.$width.'" height="'.$height.'"  alt="images"/></a></div>';
					echo $images;
				endforeach;
			?>
		</div>
