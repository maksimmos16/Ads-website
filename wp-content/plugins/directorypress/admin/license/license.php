...
class License_Handler {
	...
	const SERVER_URL = 'https://directorypress.co';
	...
	private function __construct() {
		...
		if ( is_admin() ) {
			$this->manager = new Manager( 'directorypress', 'directorypress.php', self::PARENT_SLUG );
	...
	public function start() {
		...
		->validate_with(
			array(
				'license_key' => __( 'Enter a valid license key.', 'DIRECTORYPRESS' ),
				'email'       => __( 'Enter valid/same email address used at the time of purchase.', 'DIRECTORYPRESS' ),
				'order_id'    => __( 'Enter same/valid purchase order ID.', 'DIRECTORYPRESS' ),
...
if ( '' === License_Handler::PARENT_SLUG ) {
	function add_license_menu() {
		add_options_page(
			__( 'Activate License', 'DIRECTORYPRESS' ),
			__( 'Activate License', 'DIRECTORYPRESS' ),
			'manage_options',
			License_Handler::init()->manager->page_slug,
			array( License_Handler::init()->manager, 'generate_form' )
		);
	}
