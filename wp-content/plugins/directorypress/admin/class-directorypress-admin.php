		...
		if (isset($_GET['post_type']) && $_GET['post_type'] == DIRECTORYPRESS_POST_TYPE) {
			if (!isset($_GET['package_id'])) {
				...
				if (function_exists('wpml_object_id_filter') && $sitepress && isset($_GET['trid']) && isset($_GET['lang']) && isset($_GET['source_lang'])) {
					...
					wp_redirect(add_query_arg(array('post_type' => 'dp_listing', 'package_id' => $listing->package->id, 'trid' => $_GET['trid'], 'lang' => $_GET['lang'], 'source_lang' => $_GET['source_lang']), admin_url('post-new.php')));
				} else {
...
<?php
	...
	public function enqueue_styles() {
		...
		wp_register_style('directorypress-jquery-ui-style', '//ajax.googleapis.com/ajax/libs/jqueryui/1.10.3/themes/smoothness/jquery-ui.css');
		...
		wp_enqueue_style('directorypress-jquery-ui-style');
