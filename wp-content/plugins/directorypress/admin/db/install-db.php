...
function directorypress_install_directory() {
	...
	if (!get_option('directorypress_installed_directory')) {
		...
		$wpdb->query("CREATE TABLE IF NOT EXISTS {$wpdb->directorypress_packages} (
					...
					`number_of_listings_in_package` INT(11) NOT NULL DEFAULT '1',
		...
		if (!$wpdb->get_var("SELECT id FROM {$wpdb->directorypress_packages} WHERE name = 'Basic Package'"))
			$wpdb->query("INSERT INTO `wp_directorypress_packages` (`id`, `order_num`, `name`, `description`, `package_duration`, `package_duration_unit`, `package_no_expiry`, `change_package_id`, `number_of_listings_in_package`, `can_be_bumpup`, `has_sticky`, `has_featured`, `category_number_allowed`, `location_number_allowed`, `featured_package`, `images_allowed`, `videos_allowed`, `selected_categories`, `selected_locations`, `fields`, `upgrade_meta`) VALUES(1, 1, 'Basic Package', '', 0, '', 1, 0, 1, 1, 0, 0, 97, 5, 0, 7, 1, 'a:1:{i:0;s:0:\"\";}', 'a:1:{i:0;s:1:\"2\";}', 'a:1:{i:0;s:0:\"\";}', 'a:3:{i:1;a:3:{s:5:\"price\";i:0;s:8:\"disabled\";b:1;s:7:\"raiseup\";b:1;}i:2;a:3:{s:5:\"price\";i:0;s:8:\"disabled\";b:0;s:7:\"raiseup\";b:0;}i:3;a:3:{s:5:\"price\";i:0;s:8:\"disabled\";b:0;s:7:\"raiseup\";b:0;}}');");
