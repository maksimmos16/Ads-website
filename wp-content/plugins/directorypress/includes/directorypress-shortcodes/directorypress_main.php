		...
		if (get_query_var('listing-directorypress') || (($shortcode == DIRECTORYPRESS_LISTING_SHORTCODE || $shortcode == 'directorypress-listing') && isset($shortcode_atts['listing_id']) && is_numeric($shortcode_atts['listing_id']))) {
			...
			if (count($this->listings)) {
				...
				if(isset($DIRECTORYPRESS_ADIMN_SETTINGS['directorypress_single_listing_style']) && $DIRECTORYPRESS_ADIMN_SETTINGS['directorypress_single_listing_style'] != 'default'){
					$this->template = 'partials/single-'.$DIRECTORYPRESS_ADIMN_SETTINGS['directorypress_single_listing_style'].'.php';
				}else{
					$this->template = 'partials/single.php';
				}
			...
			do_action('archive_index_page', $this, $search_args, $shortcode_atts, $map_args);
		}
	...
	public function page_title($title, $separator = '|') {
		...
		/*if (defined('WPSEO_VERSION')) {
			...
			/*	} elseif ($this->is_category) {
					...
					//$page = get_post($directorypress_object->directorypress_archive_page_id);
					//return $wpseo_front->get_title_from_options('title-' . DIRECTORYPRESS_POST_TYPE, (array) $page);
			...
			/*}else {
				...
				if ($this->is_single) {
					...
					$yoast_title = WPSEO_Meta::get_value('title', $this->listing->post->ID);
					if (!$yoast_title) {
						$yoast_title = WPSEO_Options::get('title-' . DIRECTORYPRESS_POST_TYPE);
					}
					$title = wpseo_replace_vars($yoast_title, (array)$this->listing->post);
			...
			if (directorypress_wpml_supported_settings('directorypress_directory_title')) 
				$directory_title .= directorypress_wpml_supported_settings('directorypress_directory_title');
			else
...
';
	...
	public function insert_fb_in_head() {
		...
		echo '<meta property="og:site_name" content="' . $DIRECTORYPRESS_ADIMN_SETTINGS['directorypress_directory_title'] . '" />';
