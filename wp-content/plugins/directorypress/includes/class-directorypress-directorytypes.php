	...
	if (function_exists('wpml_object_id_filter') && $sitepress) {
		if ($sitepress->get_default_language() != ICL_LANGUAGE_CODE) {
			if (isset($_GET['action']) && $_GET['action'] == 'edit' && isset($_GET['directory_id'])) {
				...
				if ($single_string_id = icl_st_is_registered_string('DirectoryPress', 'Single item of directorytype #' . $directory_id))
					icl_add_string_translation($single_string_id, ICL_LANGUAGE_CODE, $insert_update_args['single'], ICL_TM_COMPLETE);
				if ($plural_string_id = icl_st_is_registered_string('DirectoryPress', 'Plural item of directorytype #' . $directory_id))
