...
class directorypress_media_handler {
	...
	public function upload_image() {
		...
		}elseif (wp_verify_nonce($_GET['_wpnonce'], 'upload_images') && isset($_FILES['browse_file_clogo']) && ($_FILES['browse_file_clogo']['size'] > 0) && isset($_GET['post_id']) && ($post_id = $_GET['post_id'])) {
			if (($listing = directorypress_get_listing($post_id))) {
				if (directorypress_user_permission_to_edit_listing($post_id) || !$listing->post->post_author) {
		...
		}elseif (wp_verify_nonce($_GET['_wpnonce'], 'upload_images') && isset($_FILES['browse_file_cover_image']) && ($_FILES['browse_file_cover_image']['size'] > 0) && isset($_GET['post_id']) && ($post_id = $_GET['post_id'])) {
			if (($listing = directorypress_get_listing($post_id))) {
				if (directorypress_user_permission_to_edit_listing($post_id) || !$listing->post->post_author) {
		...
		if (wp_verify_nonce($_POST['_wpnonce'], 'upload_images') && isset($_POST['attachment_id']) && isset($_POST['post_id']) && ($post_id = $_POST['post_id'])) {
			if (($listing = directorypress_get_listing($post_id))) {
				if (directorypress_user_permission_to_edit_listing($post_id)) {
					...
					echo json_encode($_POST['attachment_id']);
				}
				
			}
		}elseif (wp_verify_nonce($_POST['_wpnonce'], 'upload_images') && isset($_POST['browse_file_clogo']) && isset($_POST['post_id']) && ($post_id = $_POST['post_id'])) {
			if (($listing = directorypress_get_listing($post_id))) {
				if (directorypress_user_permission_to_edit_listing($post_id)) {
					// insert attachment ID to the post meta
					add_post_meta($post_id, '_attached_image_clogo', $_POST['browse_file_clogo']);
					echo json_encode($_POST['browse_file_clogo']);
				}
			}
		}elseif (wp_verify_nonce($_POST['_wpnonce'], 'upload_images') && isset($_POST['browse_file_cover_image']) && isset($_POST['post_id']) && ($post_id = $_POST['post_id'])) {
			if (($listing = directorypress_get_listing($post_id))) {
				if (directorypress_user_permission_to_edit_listing($post_id)) {
					// insert attachment ID to the post meta
					add_post_meta($post_id, '_attached_image_cover', $_POST['browse_file_cover_image']);
					echo json_encode($_POST['browse_file_cover_image']);
				}
			}
		}
