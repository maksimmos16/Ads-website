...
add_filter( 'template_include', 'directorypress_core_load_template', 99 );
function directorypress_core_load_template( $template ) {
    global $post, $DIRECTORYPRESS_ADIMN_SETTINGS, $directorypress_object;
	$host = $_SERVER['SERVER_NAME'] . $_SERVER['REQUEST_URI'];
	if($post){
		if ( directorypress_is_listing_page() && $DIRECTORYPRESS_ADIMN_SETTINGS['directorypress_single_listing_style'] == 3) {
			$new_template = directorypress_display_template('partials/template/template-single-full.php');
			return $new_template;
			...
			if($DIRECTORYPRESS_ADIMN_SETTINGS['archive_page_style'] == 3 || $DIRECTORYPRESS_ADIMN_SETTINGS['archive_page_style'] == 4){
				$new_template = directorypress_display_template('partials/template/template-archive-full.php');
				return $new_template;
			}
		}
	}elseif (strpos($host,'profile') !== false) {
		$new_template = directorypress_display_template('partials/template/template-author.php');
		return $new_template;
	}
...
function directorypress_author_page_title($author) {
    $host = $_SERVER['SERVER_NAME'] . $_SERVER['REQUEST_URI'];
	$author_slug = basename($host);
	$author = get_user_by( 'slug', $author_slug );
	$author_name = ($author)? get_the_author_meta('display_name', $author->ID): '';
    ob_start();
	$title = bloginfo();
	return $author_name .' &#8211; '. ob_get_clean();
}

function directorypress_comment_template( $comment_template ) {
     global $post;
     if(directorypress_is_listing_page()){ 
        return DIRECTORYPRESS_TEMPLATES_PATH .'partials/template/reviews.php';
     }else{
