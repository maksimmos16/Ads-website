			...
			if(!empty($price_field_id)){
				 if($style == 1 && isset($listing->fields[$price_field_id])){
					...
					echo '<div class="directorypress-price-style2-range clearfix">';
						echo '<span class="price-range-string">'. $listing->fields[$price_field_id]->range_options_out($listing) .'</span>';
						echo '<span class="price-range-value">'. $listing->fields[$price_field_id]->renderValueOutput($listing) .'</span>';
					echo '</div>';
