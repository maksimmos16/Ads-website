...
<?php if (count($search_field->field->selection_items)): ?>
	...
	if (directorypress_is_archive_page() && $DIRECTORYPRESS_ADIMN_SETTINGS['archive_page_style'] != 2){
		$field_width = $search_field->field->fieldwidth_archive;
	}elseif(directorypress_is_archive_page() && $DIRECTORYPRESS_ADIMN_SETTINGS['archive_page_style'] == 2){
		$field_width = '100';
	}else{
		$field_width = $search_field->field->fieldwidth;
	}
	?>
	<div class="search-element-col field-id-<?php echo $search_field->field->id; ?> field-form-id-<?php echo $search_form_id; ?> unique-form-field-id-<?php echo $search_field->field->id; ?>_<?php echo $search_form_id; ?> field-type-<?php echo $search_field->field->type; ?> pull-left clearfix" style=" width:<?php echo $field_width; ?>%; padding:0 <?php echo $gap_in_fields; ?>px;">
			...
			<div class="search-checkbox-wrapper">
				...
				while ($i <= ($columns+1)): ?>
					...
					<?php foreach ($search_field->field->selection_items AS $key=>$item): ?>
						<?php if ($i == $j): ?>
							<div class="<?php if ($search_field->search_input_mode =='checkboxes'): ?>search-checkbox<?php elseif ($search_field->search_input_mode =='radiobutton'): ?>input-checkbox<?php endif; ?> ">
								<label> 
									...
									<?php endif; ?>
									<span class="checkbox-item-name"><?php echo $item; ?><?php if ($search_field->items_count && $key !== ""): if (isset($items_count_array[$key])) echo " (".$items_count_array[$key].")"; else echo " (0)"; endif; ?></span>
		...
		<?php elseif ($search_field->search_input_mode == 'selectbox'): ?>
			<select name="field_<?php echo $search_field->field->slug; ?>" class=" cs-select cs-skin-elastic directorypress-select2" style="width: 100%;">
				<option value="" <?php if (!$search_field->value) echo 'disabled selected'; ?>><?php printf(__('- Select %s -', 'DIRECTORYPRESS'), $search_field->field->name); ?></option>
				<?php foreach ($search_field->field->selection_items AS $key=>$item): ?>
