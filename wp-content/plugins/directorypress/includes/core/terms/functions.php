...
function directorypress_get_term_parents($id, $tax, $link = false, $return_array = false, $separator = '/', &$chain = array()) {
	...
	if ($link && !is_wp_error($url)) {
		$chain[] = '<li itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem"><a itemprop="item" href="' . $url . '" title="' . esc_attr(sprintf(__('View all listings in %s', 'DIRECTORYPRESS'), $name)) . '"><span itemprop="name">' . $name . '</span></a></li>';
	} else {
		$chain[] = $name;
	}
...
function directorypress_tax_dropdowns_menu_init($params) {
	...
	if ($terms) {
		...
		echo '<div id="directorypress-tax-dropdowns-wrap-' . $uID . '" class="directorypress-tax-dropdowns-wrap">';
		...
		echo '<select class="directorypress-form-control directorypress-selectmenu-' . $tax . '" data-id="' . $uID . '" data-placeholder="' . esc_attr($placeholder) . '" ' . $autocomplete_data . '>';
		foreach ($terms AS $term) {
			if ($count) {
			...
			if ($depth > 1) {
				echo _directorypress_tax_dropdowns_menu($tax, $term->term_id, $depth, 1, $term_id, $count, $exact_terms, $hide_empty);
			}
		}
		echo '</select>';
		echo '</div>';
		...
		if ($terms) {
			...
			$option_id_inc = uniqid();
				...
				if ($depth > $current_level) {
					echo _directorypress_tax_dropdowns_menu($tax, $term->term_id, $depth, $current_level, $term_id, $count, $exact_terms, $hide_empty);
				}
			}
		}
	}
...
function directorypress_tax_dropdowns_init($tax = 'category', $field_name = null, $term_id = null, $count = true, $labels = array(), $titles = array(), $uID = null, $exact_terms = array(), $hide_empty = false) {
	...
	echo "<script>directorypress_js_instance['tax_dropdowns_" . $uID . "'] = " . json_encode($localized_data) . "</script>";
	...
	echo '<div id="directorypress-tax-dropdowns-wrap-' . $uID . '" class="' . $tax . ' cs_count_' . (int)$count . ' cs_hide_empty_' . (int)$hide_empty . ' directorypress-tax-dropdowns-wrap">';
	echo '<input type="hidden" name="' . $field_name . '" id="selected_tax[' . $uID . ']" class="selected_tax_' . $tax . '" value="' . $term_id . '" />';
	if ($exact_terms) {
		echo '<input type="hidden" id="exact_terms[' . $uID . ']" value="' . addslashes(implode(',', $exact_terms)) . '" />';
	}
	foreach ($chain AS $key=>$term_id) {
		...
		if (!empty($terms)) {
			...
			if (!empty($terms)) {
				...
				echo '<div id="wrap_chainlist_' . $level_num . '_' .$uID . '" class="'.$tax.'-input clearfix">';
					echo '<div class="row">';
						if (isset($labels[$key])) {
							echo '<div class="col-md-12">';
								echo '<label class="directorypress-submit-field-title" for="chainlist_' . $level_num . '_' . $uID . '">' . $labels[$key] . '</label>';
							echo '</div>';
						}
						echo '<div class="col-md-12">';
							echo '<select id="chainlist_' . $level_num . '_' . $uID . '" name="' . $tax . '[]" class="directorypress-form-control '.$select2_class.'">';
								echo '<option value="">' . ((isset($titles[$key])) ? $titles[$key] : __('Select term', 'DIRECTORYPRESS')) . '</option>';
								foreach ($terms AS $term) {
									if ($count)
...
function directorypress_tax_dropdowns_updateterms() {
	...
	if (!empty($terms)) {
		...
		if (!empty($terms)) {
			echo '<div id="wrap_chainlist_' . $next_level . '_' . $uID . '" class="'.$tax.'-input">';
				echo '<div class="row">';
					if ($label) {
						echo '<div class="col-md-12">';
						echo '<label class="directorypress-submit-field-title" for="chainlist_' . $next_level . '_' . $uID . '">' . $label . '</label>';
						echo '</div>';
					}
					echo '<div class="col-md-12">';
						echo '<select id="chainlist_' . $next_level . '_' . $uID . '" name="' . $tax . '[]" class="'.$select2_class.' tax-level-'.$next_level.'">';
...
function directorypress_termsSelectList($name, $tax = 'category', $selected_terms = array()) {
	echo '<select multiple="multiple" name="' . $name . '[]" class="selected_terms_list directorypress-form-control directorypress-form-group directorypress-select2" style="height: 300px">';
...
function directorypress_terms_checklist($listing) {
	...
	foreach ($terms AS $term) {
		...
		if (in_array($term->term_id, $checked_categories_ids))
			$checked = 'selected';
		echo '<option value="' . esc_attr($term->term_id) . '" ' . $checked . '>' . $term->name . '</option>';
		echo _directorypress_terms_checklist($term->term_id, $checked_categories_ids, $selected_categories);
	}
	echo '</select>';
}
function _directorypress_terms_checklist($parent = 0, $checked_categories_ids = array(), $selected_categories) {
	$html = '';
	...
	if ($terms = get_categories(array('taxonomy' => DIRECTORYPRESS_CATEGORIES_TAX, 'pad_counts' => true, 'hide_empty' => false, 'parent' => $parent, 'include' => $child_items))) {
		...
		foreach ($terms AS $term) {
			...
			$html .= '<optgroup id="' . DIRECTORYPRESS_CATEGORIES_TAX . '-' . $term->term_id . '">';
			$html .= '<option ' . $checked . ' id="in-' . DIRECTORYPRESS_CATEGORIES_TAX . '-' . $term->term_id . '" value="' . $term->term_id . '"> ' . $term->name . '</option>';
			$html .= _directorypress_terms_checklist($term->term_id, $checked_categories_ids, $selected_categories);
			$html .= '</optgroup>';
		}
	}
	return $html;
