...
class directorypress_fields_admin {
	...
	public function fields_group_list_ajax() {
		...
		echo $group_items_list;
		die();
	}
	public function fields_list_order($action = '', $new_order) {
		global $directorypress_object;
		$items = $directorypress_object->fields;
	...
	public function field_ajax_vars($id) {
		...
		echo '<script>';
			echo 'var directorypress_fields_data = ' . json_encode(
				array(
					'setting_button' => ($item->has_setting_support())? '<button type="button" class="btn btn-primary field_config" data-id="'.$id.'">'.__('Field Settings', 'directorypress-payment-manager').'</button>' : '',
					'search_setting_button' => ($item->has_search_support())? '<button type="button" class="btn btn-primary field_search_config" data-id="'.$id.'">'.__('Search Settings', 'directorypress-payment-manager').'</button>' : '',
				)
			).';
		';
		echo '</script>';
	...
	public function configure($id) {
		...
		echo '</div>';
		echo '<div class="directorypress-data-list">';
			echo '<label>'.__('Type', 'directorypress-payment-manager').'</label><span>'. $item->type.'</span>';
		echo '</div>';
		echo '<div class="directorypress-data-list">';
			echo '<label>'.__('Required?', 'directorypress-payment-manager').'</label><span>'. $is_required.'</span>';
		echo '</div>';
		echo '<div class="directorypress-data-list">';
			echo '<label>'.__('Sortable?', 'directorypress-payment-manager').'</label><span>'. $is_ordered.'</span>';
		echo '</div>';
		echo '<div class="directorypress-data-list">';
			echo '<label>'.__('Searchable?', 'directorypress-payment-manager').'</label><span>'. $searchable.'</span>';
		echo '</div>';
		echo '<div class="directorypress-data-list">';
			echo '<label>'.__('Show on Map?', 'directorypress-payment-manager').'</label><span>'. $on_map.'</span>';
		echo '</div>';
		echo '<div class="directorypress-data-list">';
			echo '<label>'.__('Show on Detail Page?', 'directorypress-payment-manager').'</label><span>'. $on_listing_page .'</span>';
		echo '</div>';
		echo '<div class="directorypress-data-list">';
			echo '<label>'.__('Show on Grid View?', 'directorypress-payment-manager').'</label><span>'. $on_exerpt_page .'</span>';
		echo '</div>';
		echo '<div class="directorypress-data-list">';
			echo '<label>'.__('Show on List View?', 'directorypress-payment-manager').'</label><span>'. $on_exerpt_page_list .'</span>';
		echo '</div>';
		echo '<div class="directorypress-data-list assign_field_group">';
			...
			echo '<select id="assign_field_group" data-id="' . $item->id . '" name="group_id_' . $item->id . '">';
	...
	public function group_configure($id) {
		...
		echo '</div>';
		echo '<div class="directorypress-data-list">';
			echo '<label>'.__('Show in Tabs?', 'directorypress-payment-manager').'</label><span>'. $on_tab.'</span>';
		echo '</div>';
		echo '<div class="directorypress-data-list">';
			echo '<label>'.__('Group Style', 'directorypress-payment-manager').'</label><span>'. __('Style', 'directorypress-payment-manager'). ' ' . $item->group_style.'</span>';
		echo '</div>';
		...
		$output .= '<div class="dp-list-section">';
			foreach ($items->fields_array as $id=>$item) {
				$output .= '<div class="row dp-list-row">';
					$output .= '<input type="hidden" class="field_weight_id" value="'.$item->id.'" />';
					$output .= '<div class="col-md-10 txt-left dp-list-item"><i class="fas fa-arrows-alt"></i><span class="badge badge-default">' . $item->id .'</span> ' .$item->name. ' <span class="label label-default">' . $item->type .'</span></div>';
					$output .= '<div class="col-md-2 text-right dp-list-item"><a class="fields_configure" href="#" data-toggle="modal" data-target="#fields_configure" data-id="'.$item->id.'" data-type="'.$item->type.'"><i class="fas fa-cog"></i>'.__('Configure', 'directorypress-payment-manager').'</a></div>';
				$output .= '</div>';
			}
		$output .= '</div>';
	...
	public function group_table($items) {
		...
		$output .= '<div class="dp-list-section">';
			foreach ($items->fields_groups_array as $id=>$item) {
				$output .= '<div class="row dp-list-row">';
					$output .= '<input type="hidden" class="field_group_weight_id" value="'.$item->id.'" />';
					$output .= '<div class="col-md-10 txt-left dp-list-item"><i class="fas fa-arrows-alt"></i>'.$item->id.' - ' .$item->name. '</div>';
					$output .= '<div class="col-md-2 text-right dp-list-item"><a class="groups_configure" href="#" data-toggle="modal" data-target="#groups_configure" data-id="'.$item->id.'"><i class="fas fa-cog"></i>'.__('Configure', 'directorypress-payment-manager').'</a></div>';
				$output .= '</div>';
			}
		$output .= '</div>';
	...
	public function add_or_edit_fields($id = null, $action = '') {
		...
		if (!$field = $fields->get_field_by_id($id)) {
			if (isset($_POST['type']) && $_POST['type']) {
				$field_class_name = 'directorypress_field_' . $_POST['type'];
...
class directorypress_manage_fields_table extends WP_List_Table {
	...
	public function column_field_name($item) {
		...
		if (!$item['is_core_field'])
			$actions['delete'] = sprintf('<a href="?page=%s&action=%s&field_id=%d">' . __('Delete', 'DIRECTORYPRESS') . '</a>', $_GET['page'], 'delete', $item['id']);
		return sprintf('%1$s %2$s', sprintf('<a href="?page=%s&action=%s&field_id=%d">' . $item['field_name'] . '</a><input type="hidden" class="field_weight_id" value="%d" />', $_GET['page'], 'edit', $item['id'], $item['id']), $this->row_actions($actions));
	}
	...
	public function column_icon_image($item) {
		if ($item['icon_image'])
			return '<span class="directorypress-icon-tag directorypress-fa ' . $item['icon_image'] . '"></span>';
		else
			return ' ';
	}
