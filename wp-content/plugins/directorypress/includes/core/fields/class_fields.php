...
class directorypress_fields {
	...
	public function save_values($post_id, $categories_ids, $package_id = null, &$errors, $data) {
...
class directorypress_field {
	...
	public function validation() {
		...
		if ($process_field->is_slug()) {
			...
			if ($wpdb->get_results($wpdb->prepare("SELECT * FROM {$wpdb->directorypress_fields} WHERE slug=%s AND id!=%d", $_POST['slug'], $this->id), ARRAY_A)
