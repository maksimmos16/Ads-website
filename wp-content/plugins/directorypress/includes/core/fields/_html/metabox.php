	...
	foreach ($fields AS $field): 
		//if (!$field->is_core_field )
			if (!$field->is_categories() || $field->categories === array()) { ?>
				fields_in_categories[<?php echo $field->id?>] = [];
		<?php } else { ?>
				fields_in_categories[<?php echo $field->id?>] = [<?php echo implode(',', $field->categories); ?>];
		<?php } ?>
	<?php endforeach; ?>
			...
			function hideShowFields() {
				...
				$.each(fields_in_categories, function(index, value) {
					...
					if (value != undefined) {
						...
						if ((value.length == 0 || show_field) && $(".field-input-item.field-id-"+index).length)
							$(".field-input-item.field-id-"+index).show();
					}
				});
			}
		});
...
<div class="alert alert-info"><?php _e('Fields can be depended on select category', 'DIRECTORYPRESS'); ?></div>
<?php
	foreach ($fields AS $field) {
		if (!$field->is_core_field){
			$field->renderInput();
