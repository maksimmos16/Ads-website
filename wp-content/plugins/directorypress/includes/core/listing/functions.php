			...
			if ($total_pages > 1){
				...
				if (!$show_more_button) {
					...
					if ($total_pages > 10 && $current_page > 10)
						echo '<li class="directorypress-inactive previous_line"><a href="' . get_pagenum_link($previous_line_page) . '" title="' . esc_attr__('Previous Line', 'DIRECTORYPRESS') . '" data-page=' . $previous_line_page . ' data-handler-hash=' . $hash . '><<</a></li>' ;
			
					if ($total_pages > 3 && $current_page > 1)
						echo '<li class="directorypress-inactive previous"><a href="' . get_pagenum_link($previous_page) . '" title="' . esc_attr__('Previous Page', 'DIRECTORYPRESS') . '" data-page=' . $previous_page . ' data-handler-hash=' . $hash . '><</i></a></li>' ;
					...
					while ($count < $end) {
						...
						if ($count == $current_page)
							...
							echo '<li class="directorypress-inactive"><a href="' . get_pagenum_link($count) . '" data-page=' . $count . ' data-handler-hash=' . $hash . '>' . $count . '</a></li>' ;
					}
			
					if ($total_pages > 3 && $current_page < $total_pages)
						echo '<li class="directorypress-inactive next"><a href="' . get_pagenum_link($next_page) . '" title="' . esc_attr__('Next Page', 'DIRECTORYPRESS') . '" data-page=' . $next_page . ' data-handler-hash=' . $hash . '>></i></a></li>' ;
			
					if ($total_pages > 10 && $current_line < $total_lines)
						echo '<li class="directorypress-inactive next_line"><a href="' . get_pagenum_link($next_line_page) . '" title="' . esc_attr__('Next Line', 'DIRECTORYPRESS') . '" data-page=' . $next_line_page . ' data-handler-hash=' . $hash . '>>></a></li>' ;
					...
					echo '<button class="btn btn-primary btn-lg btn-block directorypress-show-more-button directorypress-new-btn-4 ' . $scrolling_paginator_class . '" data-handler-hash="' . $hash . '">' .__('Load More', 'DIRECTORYPRESS') . '</button>';
				}
			}
		}
	}
...
function directorypress_social_sharing_display($post_id, $button) {
	...
	if ($DIRECTORYPRESS_ADIMN_SETTINGS['directorypress_share_buttons']['enabled']) {
		...
		echo '<a href="'.$share_url.'" data-toggle="tooltip" title="'.sprintf(__('Share on %s', 'DIRECTORYPRESS'), $button).'" target="_blank"><img src="'.DIRECTORYPRESS_RESOURCES_URL.'images/social/directorypress/'.$button.'.png" /></a>';
	...
	foreach ($listing->locations AS $location){
		if($location->get_location() || $location->address_line_1){
			...
			if ($location->map_coords_1 && $location->map_coords_2){
				echo '<span class="directorypress-show-on-map" data-location-id="'. $location->id.'">';
			}
			if($location->get_location()){
				echo $location->get_location();
			}elseif($location->address_line_1){
				echo $location->address_line_1;
			}
			if ($location->map_coords_1 && $location->map_coords_2){
				echo '</span>';
			}
...
function directorypress_author_verified_function($listing){
	...
	if($status == 'verified'){
		$html = '<span class="verified"><i class="fas fa-check"></i></span>';
	}else{
		$html = '<span class="unverified"><i class="fas fa-check"></i></span>';
	}
	echo $html;
}
	...
	if($view_swither_panel_style == 4){
		if (!$public_handler->args['hide_count'] && !$public_handler->args['scroll']){
			echo '<div class="directorypress-found-listings">';
				echo sprintf(esc_html__('%s Result Found', "DIRECTORYPRESS"),$public_handler->query->found_posts);
			echo '</div>';
		}
	}else{
		if (!$public_handler->args['hide_count'] && !$public_handler->args['scroll']){
			echo '<div class="directorypress-found-listings">';
				echo esc_html__('Result Found', "DIRECTORYPRESS").'<span class="badge">'.$public_handler->query->found_posts.'</span>';
			echo '</div>';
		}
	}
}
function directorypress_listing_view_switcher_function($public_handler, $listing_style_to_show){
	...
	$view_swither_panel_style = (isset($DIRECTORYPRESS_ADIMN_SETTINGS['view_switther_panel_style']))? $DIRECTORYPRESS_ADIMN_SETTINGS['view_switther_panel_style'] : 1;
	if($view_swither_panel_style == 4){
		$grid_icon = 'fas fa-th-large';
		$list_icon = 'fas fa-bars';
	}else{
		$grid_icon = 'fas fa-th';
		$list_icon = 'fas fa-bars';
	}
	if ($listing_style_to_show == 'show_list_style'){
		$btn_list_classes = 'btn-primary';
		$btn_grid_classes = 'btn-default';
	}else{
		$btn_list_classes = 'btn-default';
		$btn_grid_classes = 'btn-primary';
	}
	if ($public_handler->args['show_views_switcher']){
		echo '<div id="view-btn-'.$public_handler->hash .'" class="directorypress-views-links pull-right">';
			echo '<div class="btn-group" role="group">';
				echo '<a class="btn directorypress-list-view-btn '.$btn_list_classes.'" href="javascript: void(0);" title="'. esc_attr__('List View', 'DIRECTORYPRESS').'" data-handler-hash="'.$public_handler->hash .'">';
					echo '<span class="'.$list_icon.'" aria-hidden="true"></span>';
				echo '</a>';
				echo '<a class="btn directorypress-grid-view-btn '.$btn_grid_classes.'" href="javascript: void(0);" title="'. esc_attr__('Grid View', 'DIRECTORYPRESS').'" data-handler-hash="'.$public_handler->hash .'" data-columns="'.$public_handler->args['listings_view_grid_columns'].'">';
					echo '<span class="'.$grid_icon.'" aria-hidden="true"></span>';
				echo '</a>';
			echo '</div>';
			directorypress_map_toggle_function();
		echo '</div>';
	}
}
...
function directorypress_listing_ordering_function($public_handler){
	global $DIRECTORYPRESS_ADIMN_SETTINGS;
	$view_swither_panel_style = (isset($DIRECTORYPRESS_ADIMN_SETTINGS['view_switther_panel_style']))? $DIRECTORYPRESS_ADIMN_SETTINGS['view_switther_panel_style'] : 1;
	if($view_swither_panel_style != 3 && $view_swither_panel_style != 4 ){
		if(isset($public_handler->args['listing_order_by_txt']) && (!empty($public_handler->args['listing_order_by_txt']))){
			$order_by_txt = $public_handler->args['listing_order_by_txt'].':';
		}else{
			$order_by_txt = esc_html__('Sort By:', 'DIRECTORYPRESS');
		}
	}else{
		$order_by_txt = '';
	}
	if(!$public_handler->args['hide_order']){
		...
		if ($ordering['struct']){
			echo '<div class="directorypress-orderby-links-label">';
				if($view_swither_panel_style != 3 || $view_swither_panel_style != 4){ echo $order_by_txt; }
			echo '</div>';
			...
			echo '<select class="directorypress-sorting directorypress-select2">';
				foreach ($ordering['struct'] AS $field_slug=>$link){
					...
					echo '<option class="" value="'.$link['url'].'" data-handler-hash="'.$public_handler->hash .'" data-orderby="'.$field_slug.'" data-order="'.$link['order'].'" '.$selected.'>'.$link['field_name'].'</option>';
				}
			echo '</select>';
			echo '</div>';
		}
...
function directorypress_listing_submit_admin_info_function($field = ''){
	...
	if(!empty($field) && current_user_can('administrator')){
		echo '<a class="admin_info_link" href="#" data-toggle="modal" data-target="#admin_info_modal" data-field="'.$field.'"><i class="fas fa-info"></i> '.esc_html__('Admin Docs', 'DIRECTORYPRESS').'</a>';	
	}
}
	...
	if(!empty($field)){
		echo '<ul>';
			if(!empty($directorypress_admin_info_strings[$field]['translation'])){
				echo '<li>'. $directorypress_admin_info_strings[$field]['translation'] .'</li>';
			}
			if(!empty($directorypress_admin_info_strings[$field]['admin_setting'])){
				echo '<li>'. $directorypress_admin_info_strings[$field]['admin_setting'] .'</li>';
			}
			if(!empty($directorypress_admin_info_strings[$field]['remote_tutorial_url'])){
				echo '<li>'. $directorypress_admin_info_strings[$field]['remote_tutorial_url'] .'</li>';
			}
			if(!empty($directorypress_admin_info_strings[$field]['remote_tutorial_url2'])){
				echo '<li>'. $directorypress_admin_info_strings[$field]['remote_tutorial_url2'] .'</li>';
			}
			if(!empty($directorypress_admin_info_strings[$field]['remote_tutorial_url3'])){
				echo '<li>'. $directorypress_admin_info_strings[$field]['remote_tutorial_url3'] .'</li>';
			}
			if(!empty($directorypress_admin_info_strings[$field]['remote_tutorial_url4'])){
				echo '<li>'. $directorypress_admin_info_strings[$field]['remote_tutorial_url4'] .'</li>';
			}
			if(!empty($directorypress_admin_info_strings[$field]['remote_tutorial_url5'])){
				echo '<li>'. $directorypress_admin_info_strings[$field]['remote_tutorial_url5'] .'</li>';
			}
			if(!empty($directorypress_admin_info_strings[$field]['remote_tutorial_url6'])){
				echo '<li>'. $directorypress_admin_info_strings[$field]['remote_tutorial_url6'] .'</li>';
			}
			if(!empty($directorypress_admin_info_strings[$field]['remote_tutorial_url7'])){
				echo '<li>'. $directorypress_admin_info_strings[$field]['remote_tutorial_url7'] .'</li>';
			}
			if(!empty($directorypress_admin_info_strings[$field]['remote_tutorial_url8'])){
				echo '<li>'. $directorypress_admin_info_strings[$field]['remote_tutorial_url8'] .'</li>';
			}
			if(!empty($directorypress_admin_info_strings[$field]['remote_tutorial_url9'])){
				echo '<li>'. $directorypress_admin_info_strings[$field]['remote_tutorial_url9'] .'</li>';
			}
			if(!empty($directorypress_admin_info_strings[$field]['remote_tutorial_url10'])){
				echo '<li>'. $directorypress_admin_info_strings[$field]['remote_tutorial_url10'] .'</li>';
			}
		echo '</ul>';
	}
	die();
...
function directorypress_listing_submit_required_lable_function($field){
	if ($field->is_this_field_requirable() && $field->is_required){
		echo '<span class="lable label-danger">'. __('Required', 'directorypress-frontend').'</span>';
	}

}
function directorypress_map_toggle_function(){
	global $DIRECTORYPRESS_ADIMN_SETTINGS;
...
function directorypress_search_toggle_function(){
	global $DIRECTORYPRESS_ADIMN_SETTINGS;
	$view_swither_panel_style = (isset($DIRECTORYPRESS_ADIMN_SETTINGS['view_switther_panel_style']))? $DIRECTORYPRESS_ADIMN_SETTINGS['view_switther_panel_style'] : 1;
...
function directorypress_listing_sorting_panel($public_handler, $listing_style_to_show){
	if ($public_handler->query->found_posts && !$public_handler->args['scroll'] && (!$public_handler->args['hide_order'] || $public_handler->args['show_views_switcher'])){
		...
		echo '<div class="row directorypress-listings-block-header switcher-panel-style-'.$view_swither_panel_style.' clearfix">';
			directorypress_search_toggle_function();
			...
			if ($public_handler->args['show_views_switcher'] || !$public_handler->args['hide_order']){
				echo '<div class="'. $view_swither_panel_style_class.' directorypress-options-links clearfix">';
