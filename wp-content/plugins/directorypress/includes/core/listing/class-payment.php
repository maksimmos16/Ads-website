...
class directorypress_listings_packages {
	...
	public function __construct() {
		...
		add_action('directorypress_listing_package_process_activation', array($this, 'listing_activate_package'));
	...
	public function process_listing_creation_for_user($package_id, $user_id = false) {
		...
		if (!$numbers[$package_id]['unlimited']) {
			update_user_meta($user_id, '_listings_number_'.$package_id, $numbers[$package_id]['number'] - 1);
	...
	?>
		...
		?>
			<tr>
				<th><label for="listings_number_<?php echo $package->id; ?>"><?php _e('Number of listings of package "'.esc_attr($package->name).'"', 'DIRECTORYPRESS'); ?></label></th>
				<td>
					<input type="text" name="_listings_number_<?php echo $package->id; ?>" id="_listings_number_<?php echo $package->id; ?>" value="<?php echo esc_attr($listings_number[$package->id]['number']); ?>" class="regular-text listings_number" />
					<p>
						<input type="checkbox" name="_listings_unlimited_<?php echo $package->id; ?>" id="_listings_unlimited_<?php echo $package->id; ?>" <?php checked($listings_number[$package->id]['unlimited'], 1); ?> value="1" />
						<label for="_listings_unlimited_<?php echo $package->id; ?>"><?php _e('Unlimited', 'DIRECTORYPRESS'); ?></label>
					</p>
				</td>
			</tr>
		<?php endforeach; ?>
