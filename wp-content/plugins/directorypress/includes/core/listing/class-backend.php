...
class directorypress_listings_admin {
	...
	public function listing_info_metabox($post) {
		...
		directorypress_display_template('directorypress-adverts/info_metabox.php', array('listing' => $listing, 'packages' => $packages));
	}
	...
	public function listing_expiry_metabox($post) {
		...
		if ($listing->status != 'expired') {
			...
			directorypress_display_template('directorypress-adverts/change_date_metabox.php', array('listing' => $listing, 'dateformat' => directorypress_dpf()));
		} else {
			...
			if (isset($directorypress_object->dashboard_page_url) && $directorypress_object->dashboard_page_url)
				echo '<br /><a href="' . directorypress_dashboardUrl(array('directorypress_action' => 'renew_listing', 'listing_id' => $listing->post->ID)) . '"><span class="directorypress-field-icon directorypress-icon-refresh"></span>' . $renew_link . '</a>';
			else
				echo '<br /><a href="' . admin_url('options.php?page=directorypress_renew&listing_id=' . $listing->post->ID) . '"><span class="directorypress-field-icon directorypress-icon-refresh"></span>' . $renew_link . '</a>';
		}
	}
	
	public function listingResurvaMetabox($post) {
		$listing = directorypress_pull_current_listing_admin();
		directorypress_display_template('directorypress-adverts/resurva_booking.php', array('listing' => $listing));
	}
	...
	public function listing_contact_metabox($post) {
		...
		directorypress_display_template('directorypress-adverts/contact_email_metabox.php', array('listing' => $listing));
	}
		...
		switch ($column) {
			case "directorypress_package":
				...
				if ($listing->package && $listing->package->is_upgradable())
					echo '<a href="' . admin_url('options.php?page=directorypress_upgrade&listing_id=' . $post_id) . '" title="' . esc_attr__('Change package', 'DIRECTORYPRESS') . '">';
				echo $listing->package->name;
				if ($listing->package && $listing->package->is_upgradable())
				...
				if ($listing->package && !$listing->package->package_no_expiry)
					echo '<br />(' . $listing->package->get_active_duration_string() . ')';
				
				if ($directorypress_object->directorytypes->isMultiDirectory()) // needs workaround
					echo '<br />' . $listing->directorytype->name;
				break;
			case "directorypress_expiration_date":
				...
				if ($listing->package && $listing->package->package_no_expiry)
					...
					if (($DIRECTORYPRESS_ADIMN_SETTINGS['directorypress_change_expiration_date'] || current_user_can('manage_options')) && $listing->status == 'active')
						echo '<a href="' . admin_url('options.php?page=directorypress_changedate&listing_id=' . $post_id) . '" title="' . esc_attr__('change expiration date', 'DIRECTORYPRESS') . '">' . date_i18n(get_option('date_format') . ' ' . get_option('time_format'), intval($listing->expiration_date)) . '</a>';
					else
					...
					if ($listing->status == 'expired') {
						...
						echo '<br /><a href="' . admin_url('options.php?page=directorypress_renew&listing_id=' . $post_id) . '"><span class="directorypress-field-icon directorypress-icon-refresh"></span>' . $renew_link . '</a>';
			...
			case "directorypress_notice_to_admin":
					...
					if(metadata_exists('post', $post_id, '_notice_to_admin' ) ) {
						...
						echo '<a href="' . admin_url('options.php?page=directorypress_author_note_to_admin&listing_id=' . $post_id) . '">' . __('Note', 'DIRECTORYPRESS') . '</a>';
					}
				break;
	...
	public function posts_filter_dropdown() {
		...
		if ($directorypress_object->directorytypes->isMultiDirectory()) {
			...
			foreach ($directorypress_object->directorytypes->directorypress_array_of_directorytypes AS $directorytype)
				echo '<option ' . selected(directorypress_get_input_value($_GET, 'directorypress_directory_filter'), $directorytype->id, false ) . 'value="' . $directorytype->id . '">' . $directorytype->name . '</option>';
			echo '</select>';
		}
		...
		foreach ($directorypress_object->packages->packages_array AS $package)
			echo '<option ' . selected(directorypress_get_input_value($_GET, 'directorypress_package_filter'), $package->id, false ) . 'value="' . $package->id . '">' . $package->name . '</option>';
		echo '</select>';
	...
	public function raiseUpListing() {
		if (isset($_GET['listing_id']) && ($listing_id = $_GET['listing_id']) && is_numeric($listing_id) && directorypress_user_permission_to_edit_listing($listing_id)) {
			if ($this->get_current_listing($listing_id) && $this->current_listing->status == 'active') {
				...
				directorypress_display_template('directorypress-adverts/raise_up.php', array('listing' => $this->current_listing, 'referer' => $referer, 'action' => $action));
			} else
				exit();
		} else
			exit();
	...
	public function renewListing() {
		if (isset($_GET['listing_id']) && ($listing_id = $_GET['listing_id']) && is_numeric($listing_id) && directorypress_user_permission_to_edit_listing($listing_id)) {
			if ($this->get_current_listing($listing_id)) {
				...
				directorypress_display_template('directorypress-adverts/renew.php', array('listing' => $this->current_listing, 'referer' => $referer, 'action' => $action));
			} else
				exit();
		} else
			exit();
	...
	public function change_listing_expiry_page() {
		if (isset($_GET['listing_id']) && ($listing_id = $_GET['listing_id']) && is_numeric($listing_id) && directorypress_user_permission_to_edit_listing($listing_id)) {
			if ($this->get_current_listing($listing_id)) {
				...
				directorypress_display_template('directorypress-adverts/change_date.php', array('listing' => $this->current_listing, 'referer' => $referer, 'action' => $action, 'dateformat' => directorypress_dpf()));
			} else
				exit();
		} else
			exit();
	...
	public function listing_upgrade_page() {
		...
		if (isset($_GET['listing_id']) && ($listing_id = $_GET['listing_id']) && is_numeric($listing_id) && directorypress_user_permission_to_edit_listing($listing_id)) {
			if ($this->get_current_listing($listing_id)) {
				...
				directorypress_display_template('directorypress-adverts/upgrade.php', array('listing' => $this->current_listing, 'referer' => $referer, 'action' => $action, 'packages' => $directorypress_object->packages));
			} else
				exit();
		} else
			exit();
	...
	public function listing_bulk_upgrade_page() {
		...
		if (isset($_GET['listings_ids'])) {
			...
			directorypress_display_template('directorypress-adverts/upgrade_bulk.php', array('listings_ids' => $listings_ids, 'referer' => $referer, 'action' => $action, 'packages' => $directorypress_object->packages));
		} else
			exit();
	}
	...
	public function authornotetoAdmin() {
		if (isset($_GET['listing_id']) && ($listing_id = $_GET['listing_id']) && is_numeric($listing_id) && is_admin()) {
			if ($this->get_current_listing($listing_id)) {
				$referer = wp_get_referer();
				$action = 'show';
				directorypress_display_template('directorypress-adverts/author_notice_to_admin.php', array('listing' => $this->current_listing, 'referer' => $referer, 'action' => $action));
			}
		}
	}
	...
	public function listing_validation($data, $postarr) {
		...
		if ($data['post_type'] == DIRECTORYPRESS_POST_TYPE) {
			...
			$directorypress_object->fields->save_values($this->current_listing->post->ID, $post_categories_ids, $this->current_listing->package->id, $errors, $data);
			...
			if ($this->current_listing->package->images_allowed || $this->current_listing->package->videos_allowed) {
				if ($validation_results = $directorypress_object->media_handler_property->validate_attachments($this->current_listing->package, $errors))
					$directorypress_object->media_handler_property->save_attachments($this->current_listing->package, $this->current_listing->post->ID, $validation_results);
			}
			...
			if(isset($_POST['facebook_link'])){
				if(!metadata_exists('post', $this->current_listing->post->ID, 'facebook_link' ) ) {
					add_post_meta($this->current_listing->post->ID, 'facebook_link', $_POST['facebook_link']);
				}else{
					update_post_meta($this->current_listing->post->ID, 'facebook_link', $_POST['facebook_link']);
				}
			}
						
			if(isset($_POST['twitter_link'])){
				if(!metadata_exists('post', $this->current_listing->post->ID, 'twitter_link' ) ) {
					add_post_meta($this->current_listing->post->ID, 'twitter_link', $_POST['twitter_link']);
				}else{
					update_post_meta($this->current_listing->post->ID, 'twitter_link', $_POST['twitter_link']);
				}
			}
						
			if(isset($_POST['linkedin_link'])){
				if(!metadata_exists('post', $this->current_listing->post->ID, 'linkedin_link' ) ) {
					add_post_meta($this->current_listing->post->ID, 'linkedin_link', $_POST['linkedin_link']);
				}else{
					update_post_meta($this->current_listing->post->ID, 'linkedin_link', $_POST['linkedin_link']);
				}
			}
						
			if(isset($_POST['youtube_link'])){
				if(!metadata_exists('post', $this->current_listing->post->ID, 'youtube_link' ) ) {
					add_post_meta($this->current_listing->post->ID, 'youtube_link', $_POST['youtube_link']);
				}else{
					update_post_meta($this->current_listing->post->ID, 'youtube_link', $_POST['youtube_link']);
				}
			}
						
			if(isset($_POST['instagram_link'])){
				if(!metadata_exists('post', $this->current_listing->post->ID, 'instagram_link' ) ) {
					add_post_meta($this->current_listing->post->ID, 'instagram_link', $_POST['instagram_link']);
				}else{
					update_post_meta($this->current_listing->post->ID, 'instagram_link', $_POST['instagram_link']);
				}
			}
	...
	public function setup_current_page_directorytype() {
		...
		if (is_admin() && $pagenow == 'post-new.php' && isset($_GET['post_type']) && $_GET['post_type'] == DIRECTORYPRESS_POST_TYPE && isset($_GET['directory_id']) && is_numeric($_GET['directory_id']) && ($directorytype = $directorypress_object->directorytypes->directory_by_id($_GET['directory_id']))) {
