...
function directorypress_dashboard_activeusers(){
		...
		echo '<div><a href="users.php">' . $user_count['total_users'] . ' ' . $users_plural . '</a> <small>(' . directorypress_online_users('count') . ' currently active)</small></div>';
}
...
function directorypress_login_form($args = array()) {
	...
	echo '
		<form name="' . $args['form_id'] . '" id="' . $args['form_id'] . '" action="' . esc_url( site_url( 'wp-login.php', 'login_post' ) ) . '" method="post" class="directorypress_login_form" role="form">
...
function directorypress_ajax_login($args = array()) {
    ...
    if ($do_check == false) {
        $response['type'] = 'error';
        $response['message'] = esc_html__('Security token validation failed!', 'directorypress-frontend');
    ...
    if ( is_wp_error($login_request) ){
		...
		$response['message'] = esc_html__('login successful!', 'directorypress-frontend');
