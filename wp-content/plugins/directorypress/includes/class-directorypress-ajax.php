...
class directorypress_ajax {
	...
	public function directorypress_handler_demand() {
		...
		switch ($post_args['handler']) {
			case "directorypress_directory_handler":
			case "directorypress_listings_handler":
				if ($post_args['handler'] == "directorypress_directory_handler"){
					$shortcode_atts = array_merge(array(
							'perpage' => (isset($post_args['is_home']) && $post_args['is_home']) ? $DIRECTORYPRESS_ADIMN_SETTINGS['directorypress_listings_number_index'] : $DIRECTORYPRESS_ADIMN_SETTINGS['directorypress_listings_number_excerpt'],
							'onepage' => 0,
							...
							'listings_view_type' => (int)$DIRECTORYPRESS_ADIMN_SETTINGS['directorypress_views_switcher_default'],
				...
				if (isset($post_args['address'])) {
					$address = apply_filters('directorypress_search_param_address', $post_args['address']);
				}
				if (isset($post_args['radius'])) {
					$radius = apply_filters('directorypress_search_param_radius', $post_args['radius']);
				}
				
				if (isset($post_args['order_by'])) {
					$_REQUEST['order_by'] = directorypress_get_input_value($post_args, 'order_by', $shortcode_atts['order_by']);
					$_REQUEST['order'] = directorypress_get_input_value($post_args, 'order', $shortcode_atts['order']);
				...
				$directorypress_handler->init($post_args);
				$directorypress_handler->hash = $post_args['hash'];
				...
				} elseif (!empty($shortcode_atts['directorytypes']) && $post_args['handler'] == 'directorypress_directory_handler') {
				...
				if (!empty($post_args['base_url']))
					$directorypress_handler->base_url = add_query_arg($base_url_args, $post_args['base_url']);
				else 
					...
					if (isset($post_args['do_append']) && $post_args['do_append']) {
						if ($directorypress_handler->listings)
							while ($directorypress_handler->query->have_posts()) {
								...
								$listings_html .= '<article id="post-' . get_the_ID() . '" class="row directorypress-listing '.$directorypress_responsive_col.' listing-post-style-'.$listing_style.'   clearfix m-grid-item ' . (($directorypress_handler->listings[get_the_ID()]->package->has_featured) ? 'directorypress-has_featured' : '') . ' ' . (($directorypress_handler->listings[get_the_ID()]->package->has_sticky) ? 'directorypress-has_sticky' : '') . '" style="padding-left:'.$grid_padding.'px; padding-right:'.$grid_padding.'px; '.$directorypress_grid_margin_bottom.'" >';
				...
				$out = array(
						...
						'map_markers' => ((!empty($post_args['with_map']) && $directorypress_handler->map) ? $directorypress_handler->map->locations_option_array : ''),
						'map_listings' => ((!empty($post_args['map_listings']) && $directorypress_handler->map) ? $directorypress_handler->map->buildListingsContent() : ''),
	...
	public function get_map_markers() {
		...
		if (isset($post_args['neLat']) && isset($post_args['neLng']) && isset($post_args['swLat']) && isset($post_args['swLng'])) {
			...
			if (!isset($post_args['directorytypes'])) {
				$post_args['directorytypes'] = $directorypress_object->current_directorytype->id;
			}
		...
		if ((!isset($post_args['without_listings']) || !$post_args['without_listings'])) {
			...
			if (isset($post_args['base_url']) && $post_args['base_url'])
				$directorypress_listings_handler->base_url = add_query_arg($base_url_args, $post_args['base_url']);
			else
	...
	public function search_by_poly() {
		...
		if (isset($post_args['geo_poly']) && $post_args['geo_poly']) {
			...
			$post_args['ajax_markers_loading'] = 0; 
			$post_args['radius'] = 0; 
			$post_args['address'] = ''; 
			$post_args['location_id'] = 0; 
			$post_args['locations'] = ''; 
			$post_args['custom_home'] = 0;
			...
			if ($validation->run()) {
				...
				if ($listing->directorypress_init_lpost_listing($validation->result_array('listing_id'))) {
					...
					if (directorypress_recaptcha_validated()) {
						...
						$body = directorypress_display_template('directorypress-emails/' . $type . '_form.php',
						array(
							'name' => $name,
							'email' => $email,
							'message' => $message,
						...
						$error = esc_attr__("Anti-bot test wasn't passed!", 'DIRECTORYPRESS');
					}
				}
			} else {
			...
			if ($validation->run()) {
				...
				if ($listing->directorypress_init_lpost_listing($validation->result_array('listing_id'))) {
					...
					if (directorypress_recaptcha_validated()) {
						...
						$body = directorypress_display_template('directorypress-emails/contact_form_widget.php',
						array(
							'name' => $name,
							'email' => $email,
							'message' => $message,
