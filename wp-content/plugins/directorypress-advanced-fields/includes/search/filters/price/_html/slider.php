...
<?php $index = $search_field->field->id . '_' . $search_form_id; ?>
...
?>
<div class="czprice-slider search-element-col field-id-<?php echo $search_field->field->id; ?> field-type-<?php echo $search_field->field->type; ?> field-form-id-<?php echo $search_form_id; ?> unique-form-field-id-<?php echo $index; ?> pull-left" style=" width:<?php echo $field_width ?>%; padding:0 <?php echo $gap_in_fields; ?>px;">
				...
				$('#range_slider_<?php echo $index; ?>').slider({
					...
					$.each(slider_params_<?php echo $index; ?>, function(index, value) {
						...
						if (index % odd_even_label == 0) {
							...
							var post_symbol_position = ' <?php echo esc_js($search_field->field->currency_symbol); ?>';
							<?php endif; ?>
							//var el = $('<label><span>|</span><span class="directorypress-range-slider-label">' + pre_symbol_position + value + post_symbol_position + '</span></label>').css(position, (100/(slider_params_<?php echo $index; ?>.length-1))*index + '%');
						} else {
