...
class classiadsproSidebarGenerator {
	...
	function get_sidebar( $post_id = null ) {
		...
		if (is_active_sidebar("alsp_listing_single") && is_page() && (has_shortcode($post->post_content, 'alsp-listing') ||  (class_exists('DirectoryPress') && directorypress_is_listing_page()))) {
		...
		if ( function_exists('is_woocommerce') && is_active_sidebar("sidebar-7") && is_woocommerce() && is_archive()) {
