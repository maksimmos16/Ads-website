<?php
	...
	global $pacz_settings;
		...
		$squeeze_sticky_header = ($pacz_settings['squeeze-sticky-header']) ? 1 : 0;
	...
	if($header_style == 'transparent') {
		  ...
		  $header_class .= ($sticky_header == 'true' || $sticky_header == 1) ? ' sticky-header' : '';
	} else {
		  $header_class = ($sticky_header == 'true' || $sticky_header == 1) ? 'block-header sticky-header' : 'block-header';
		  $header_padding_class = ($sticky_header == 'true' || $sticky_header == 1) ? 'sticky-header' : '';
	}
	$pacz_header_sticky_offset = ($post_id && !empty(get_post_meta($post_id, '_trans_header_offset', true ))) ? get_post_meta($post_id, '_trans_header_offset', true ) : $pacz_settings['sticky_header_offset'];
	if(($sticky_header == 'true' || $sticky_header == 1) && $pacz_header_sticky_offset == 0){
	...
	echo '<header id="pacz-header" class="'.esc_attr($header_class).' '.esc_attr($header_grid).' '.esc_attr($header_grid_margin).' '.$header_grid_postion.' theme-main-header pacz-header-module" data-header-style="'.esc_attr($header_style).'" data-header-structure="'.esc_attr($header_structure).'" data-transparent-skin="'.esc_attr($trans_header_skin).'" data-height="'.intval($header_height).'" data-sticky-height="'.intval($header_sticky_height).'">';
		if($boxed_header && $header_structure != 'vertical') {
			...
			echo '</div>';
			echo '<div class="classiads-menu-wrapper">';
	...
	echo '</header>';
	echo '<div id="header_placeholder"></div>';
