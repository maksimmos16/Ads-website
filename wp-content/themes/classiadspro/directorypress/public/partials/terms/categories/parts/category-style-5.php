				...
				$tcounter++;
					...
					echo '<div class="directorypress-category-item col-md-' . $instance->col . ' col-sm-' . $instance->col_tab . ' col-xs-' . $instance->col_mobile . '">';
						echo '<div id="cat-wrapper-'.$term->term_id.'" class="directorypress-category-holder clearfix">';		
							echo '<div class="directorypress-parent-category"><a href="' . get_term_link($term) . '" title="' . $term->name . '">' . $instance->termIcon($term->term_id) . $term->name .'<span class="categories-count">'.$instance->renderTermCount($term). $more_cat_icon.'</span></a></div>';
						echo '</div>';
						//if($instance->depth > 1){
							// modal
